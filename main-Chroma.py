...
def compute_std_per_label(img, labeled_objects, labels):
    ...
    """
    stds = ndimage.labeled_comprehension(
        img, labeled_objects, labels, np.std, float, 0
    )
...
def compute_outliers_mask(label_objects, labels, stds, alpha=0.1):
    ...
    mask_labels = [i for i, std in zip(labels, stds) if std < thresh]
    mask = np.isin(label_objects, mask_labels).astype(np.uint8) * 255
...
def compute_NFA(mask, chars, text_mask, threshold, alpha):
    """
    Computes the Number of False Alarms (NFA).
    """
...
def process_channel(
    ch, img, chroma_img, text_mask, alpha, threshold, output_path
):
...
def detect_parallelized(
    img_path, alpha, threshold, output_path, east_model_path
):
    """Detects outliers in an image using parallel processing 
    ...
    east_model_path = os.path.join(east_model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('east_model_path')
    parser.parse_args()
    args = parser.parse_args()
    path = args.dataset_path
    output_path = args.output_path
    east_model_path = args.east_model_path
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    for img in Path(path).glob('*.tif'):
        print(f"Processing {img}")
        detect_parallelized(str(img), 0.1, 0.01, output_path, east_model_path)
